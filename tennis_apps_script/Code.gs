...
function analyzeTennisStringAI(item) {
  try {
    ...
    var prompt = "너는 세계적인 프로 테니스 스트링 기술자이자 스포츠 의학/장비 전문 수석 테니스 코치야.\n" +
      ...
      "【분석 지침】\n" +
      "1. 스트링 종류별 물리적 특성(폴리 스트링은 15~20시간 후 텐션/탄성이 급격히 소실되는 '데드 폴리' 현상 발생, 인조쉽/천연쉽은 탄력 유지 및 보풀)을 명확히 반영할 것.\n" +
