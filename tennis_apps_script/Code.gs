...
function analyzeTennisStringAI(item) {
  try {
    ...
    var prompt = "너는 세계적인 프로 테니스 스트링 기술자이자 동호인 맞춤 스포츠 의학/장비 전문 수석 테니스 코치야.\n" +
      "사용자(여성 테니스 동호인)가 입력한 라켓/스트링 정보와 교체 이후 누적된 테니스 훈련 및 경기 기록을 정밀 분석하여, 스트링의 텐션 로스 상태와 최적의 교체 시점을 진단해줘.\n\n" +
      "【장착 스트링 사양】\n" +
      ...
      "- 최근 메모 및 타구 피드백: " + JSON.stringify(stats.logsSince.slice(-10).map(function(l){ return l.date + '(' + l.category + '): ' + (l.notes || '정상'); })) + "\n\n" +
      "【분석 지침】\n" +
      "1. [동호인 맞춤 가이드]: 선수가 아닌 '여성 테니스 동호인(레크리에이션/클럽 플레이어)' 기준에 맞춰 현실적인 2~3개월(40~50시간) 주기를 기준으로 평가할 것.\n" +
      "2. 테니스 엘보/손목 부상 방지 관점에서 현재 스트링의 충격 흡수율 저하 위험도와 반발력 변화를 점검할 것.\n" +
